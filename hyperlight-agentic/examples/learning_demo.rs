...
fn demo_curriculum_learning() {
    ...
    if let Some(stage) = curriculum.get_next_stage(&agent1) {
        ...
        match result {
            hyperlight_agentic::CurriculumResult::RetryNeeded { attempts_remaining, current_score, .. } => {
        ...
        match result {
            hyperlight_agentic::CurriculumResult::StageCompleted { score, next_stage, .. } => {
    ...
    if let Some(stage) = curriculum.get_next_stage(&agent2) {
        ...
        for i in 1..=3 {
            ...
            match result {
                hyperlight_agentic::CurriculumResult::RetryNeeded { attempts_remaining, .. } => {
                    println!("    ✗ Attempt {}: 40% ({} tries left)", i, attempts_remaining);
                }
                hyperlight_agentic::CurriculumResult::StageFailed { attempts, best_score, .. } => {
...
async fn demo_emergent_behavior() {
    ...
    for behavior in &behaviors {
        ...
        match &behavior.pattern {
            hyperlight_agentic::BehaviorPattern::SpontaneousCoordination { action_sequence } => {
                println!("      Actions: {:?}", &action_sequence[..action_sequence.len().min(3)]);
            }
            hyperlight_agentic::BehaviorPattern::RoleDifferentiation { roles, .. } => {
                println!("      Roles: {:?}", roles);
            }
            hyperlight_agentic::BehaviorPattern::NovelStrategy { solution_path, .. } => {
