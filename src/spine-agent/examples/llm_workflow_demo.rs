...
use spine_agentic::workflow::{
    StepKind, StepStatus, WorkflowBuilder, WorkflowEngine, WorkflowStep,
};
    ...
    fn auto() -> Self {
        ...
        if let Ok(key) = std::env::var("ANTHROPIC_API_KEY") {
            if !key.is_empty() {
                ...
                return Self {
                    backend: LlmBackend::Anthropic(key),
                };
            }
        }
        println!("  [LLM] No API key - using offline backend");
        Self {
            backend: LlmBackend::Offline,
        }
    }
...
fn main() {
    ...
    // Build workflow DAG
    let mut builder =
        WorkflowBuilder::new("llm-research").description("LLM-Powered Research Pipeline");

    let parse_idx = builder.add_step(WorkflowStep::new(
        "parse",
        StepKind::Custom {
            kind: "parse".into(),
    ...
    let summarize_idx = builder.add_step(WorkflowStep::new(
        "summarize",
        StepKind::AgentCall {
            capability: "llm-summarize".into(),
        },
    ));
    let extract_idx = builder.add_step(WorkflowStep::new(
        "extract",
        StepKind::AgentCall {
            capability: "llm-extract".into(),
        },
    ));
    let classify_idx = builder.add_step(WorkflowStep::new(
        "classify",
        StepKind::AgentCall {
    ...
    let compile_idx = builder.add_step(WorkflowStep::new(
        "compile",
        StepKind::Script {
    ...
    engine
        .complete_step(&wf_id, parse_idx, serde_json::json!({"ok": true}))
        .unwrap();
    ...
    engine
        .complete_step(&wf_id, summarize_idx, serde_json::json!({"s": summary}))
        .unwrap();
    ...
    engine
        .complete_step(&wf_id, extract_idx, serde_json::json!({"e": entities}))
        .unwrap();
    ...
    engine
        .complete_step(&wf_id, classify_idx, serde_json::json!({"c": cat}))
        .unwrap();

    engine.start_step(&wf_id, aggregate_idx, None).unwrap();
    engine
        .complete_step(&wf_id, aggregate_idx, serde_json::json!({"merged": true}))
        .unwrap();
    ...
    engine
        .complete_step(&wf_id, compile_idx, serde_json::json!({"r": report}))
        .unwrap();
    ...
    if let Some(wf) = engine.get(&wf_id) {
        ...
        println!(
            "Completed: {}",
            wf.step_counts().get(&StepStatus::Completed).unwrap_or(&0)
        );
    }
