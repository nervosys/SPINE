...
async fn main() -> anyhow::Result<()> {
    ...
    // 1. Connect to the Hyperlight Core
    let mut client = AgentClient::connect("127.0.0.1:8080").await?;
    println!("Connected to Hyperlight Core");

    // 2. Store some knowledge
    println!("Storing knowledge about Hyperlight...");
    ...
    let results = client.query_knowledge("hyperlight", vec![], 10).await?;
    for res in results {
