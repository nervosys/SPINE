// =============================================================================
// HYPERLIGHT CLUSTER - Distributed Agent Coordination
// =============================================================================
//
// This module provides distributed coordination for Hyperlight agents:
//
...
/// A node in the Hyperlight cluster
...
impl ClusterNode {
                    ...
                    tokio::spawn(async move {
                        ...
                        if socket.read_to_end(&mut buf).await.is_ok() {
                            if let Ok(msg) = serde_json::from_slice::<ClusterMessage>(&buf) {
                                match msg {
                                    ClusterMessage::Heartbeat { node_id, load, sessions_count } => {
                                    ...
                                    ClusterMessage::SessionTransferRequest { session_id, from_node, to_node } => {
                                        if to_node == id {
                                            let _ = event_tx.send(ClusterEvent::SessionTransferRequested {
                                                session_id,
                                                from_node,
                                            });
                                        }
                                    ...
                                    ClusterMessage::SearchRequest { query, request_id, origin_node } => {
                                        if origin_node != id {
                                    ...
                                    ClusterMessage::SearchResponse { request_id, results, node_id } => {
    ...
    pub async fn send_search_results(&self, target_node_id: NodeId, request_id: String, results: serde_json::Value) -> Result<()> {
    ...
    pub fn select_node(&self) -> Option<NodeId> {
        ...
        match self.lb_strategy {
            LoadBalancingStrategy::RoundRobin => {
                let idx = self.rr_index.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    ...
    pub fn register_session(&self, session_id: SessionId, node_id: NodeId) {
        let now = current_time_millis();
        self.sessions.insert(session_id.clone(), SessionLocation {
            session_id,
            node_id,
            created_at: now,
            last_accessed: now,
    ...
    pub async fn initiate_session_transfer(&self, session_id: SessionId, to_node: NodeId) -> Result<()> {
    ...
    pub async fn send_session_data(&self, session_id: SessionId, data: Vec<u8>) -> Result<()> {
        let msg = ClusterMessage::SessionTransferData {
            session_id,
            data,
        };
        self.broadcast_message(msg).await
    ...
    pub async fn start_election(&self) -> Result<bool> {
        ...
        *term += 1;
        ...
        if is_leader {
            ...
            *leader = Some(self.id);
            log::info!("This node ({}) elected as leader for term {}", self.id, current_term);
        } else {
            let mut leader = self.leader.write().await;
            *leader = Some(lowest_id);
            log::info!("Node {} elected as leader for term {}", lowest_id, current_term);
        }
...
/// Client for connecting to a Hyperlight cluster
...
mod tests {
    ...
    fn test_session_registration() {
        ...
        // Register self first
        node.nodes.insert(node_id, NodeInfo {
            id: node_id,
            address: addr,
            status: NodeStatus::Healthy,
            last_heartbeat: current_time_millis(),
