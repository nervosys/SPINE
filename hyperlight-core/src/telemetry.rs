...
lazy_static! {
    ...
    pub static ref SESSIONS_ACTIVE: Counter = register_counter!(
        Opts::new("hyperlight_sessions_active", "Number of active browser sessions")
    ).unwrap();
    
    pub static ref COMMANDS_TOTAL: Counter = register_counter!(
        Opts::new("hyperlight_commands_total", "Total number of commands processed")
    ).unwrap();
    
    pub static ref COMMAND_LATENCY: Histogram = register_histogram!(
        HistogramOpts::new("hyperlight_command_latency_seconds", "Latency of command processing")
            .buckets(vec![0.001, 0.005, 0.01, 0.05, 0.1, 0.5, 1.0, 2.0, 5.0])
    ).unwrap();
    
    pub static ref PROTOCOL_MORPHS: Counter = register_counter!(
        Opts::new("hyperlight_protocol_morphs_total", "Total number of protocol morphing events")
    ).unwrap();
}
...
pub fn init_telemetry(service_name: &str) -> anyhow::Result<()> {
    ...
    let filter = EnvFilter::from_default_env()
        .add_directive("hyperlight_core=debug".parse()?);
