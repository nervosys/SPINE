...
use chrono::Utc;
use spine_agentic::{
    create_agent, create_agent_with_capabilities, Action, AgentCapability, Condition, Goal,
    KnowledgeGraph, KnowledgeNode, MessageContent, OutputType, Plan, ResourceLocator,
    SemanticQuery, SwarmRole, SwarmTask, TrustLevel,
};
use uuid::Uuid;
...
fn demo_resource_locators() {
    ...
    // Content-addressed
    let content_loc = ResourceLocator::ContentAddress("sha256:a1b2c3d4e5f6...".to_string());
    println!();
...
fn demo_semantic_queries() {
    ...
    println!(
        "  Output: {:?}, Confidence: {}+",
        query1.output_type, query1.confidence_threshold
    );
...
fn demo_planning() {
    ...
    let step1 = plan.add_step(spine_agentic::PlanStep {
        ...
        preconditions: vec![],
        postconditions: vec![Condition::ResourceExists(ResourceLocator::semantic(
            "research database",
        ))],
    ...
    let step2 = plan.add_step(spine_agentic::PlanStep {
        ...
        preconditions: vec![Condition::ResourceExists(ResourceLocator::semantic(
            "research database",
        ))],
        postconditions: vec![],
    ...
    // Conditional action
    let _conditional = Action::Branch {
...
fn demo_agent_communication() {
    ...
    let messages: Vec<(&str, MessageContent)> = vec![
        (
            "Query",
            MessageContent::Query(SemanticQuery {
        ...
        (
            "Action Request",
            MessageContent::ActionRequest(Box::new(Action::Navigate(ResourceLocator::semantic(
                "quantum papers",
            )))),
        ),
        (
        ...
        (
            "Swarm Invite",
            MessageContent::SwarmInvite(SwarmTask {
                id: Uuid::new_v4(),
