...
async fn demo_versioning() {
    ...
    for migration in &path {
        ...
        controller
            .apply_migration(agent_id, migration)
            .await
            .unwrap();
    }
...
async fn demo_semantic_routing() {
    ...
    router.add_route(SemanticRoute {
        id: Uuid::new_v4(),
        pattern: SemanticPattern::Topic(vec![
            "code".into(),
            "programming".into(),
            "implementation".into(),
        ]),
    ...
    println!(
        "\n  CodingAgent subscribed to bug reports: {}",
        &sub_id.to_string()[..8]
    );
...
async fn demo_consensus() {
    ...
    for (voter, decision, weight, reasoning) in votes {
        engine
            .vote(proposal_id, voter, decision.clone(), weight, reasoning)
            .unwrap();
    ...
    if status == ConsensusStatus::Accepted || status == ConsensusStatus::Voting {
        ...
        println!(
            "  ✓ Decision committed at {}",
            committed.committed_at.format("%H:%M:%S")
        );
        println!("    Accept votes: {}", committed.vote_summary.accept_count);
        println!("    Reject votes: {}", committed.vote_summary.reject_count);
        println!(
            "    Accept weight: {:.1}/{:.1}",
            committed.vote_summary.accept_weight, committed.vote_summary.total_weight
        );
    }
...
async fn demo_introspection() {
    ...
    println!(
        "  Added breakpoint: {} (on fetch_data action)",
        &bp_id.to_string()[..8]
    );
    ...
    println!(
        "  Added watcher: {} (on knowledge_count)",
        &watcher_id.to_string()[..8]
    );
    ...
    let events = vec![
        (
            ...
            serde_json::json!({"agent": "ResearchAgent"}),
        ),
        (
            TraceEventType::IntentionCreated {
                goal: "Analyze market data".into(),
            },
            serde_json::json!({}),
        ),
        (TraceEventType::PlanningStarted, serde_json::json!({})),
        (
            TraceEventType::PlanningCompleted { steps: 3 },
            serde_json::json!({"duration_ms": 15}),
        ),
        (
            TraceEventType::ActionStarted {
                action: "fetch_data".into(),
            },
            serde_json::json!({"source": "api.market.com"}),
        ),
        (
            TraceEventType::ActionCompleted {
                result: "200 OK".into(),
            },
            serde_json::json!({"records": 1500}),
        ),
        (
            TraceEventType::KnowledgeUpdate {
                node_id: "market-2025".into(),
            },
            serde_json::json!({}),
        ),
        (
            TraceEventType::MessageSent { to: Uuid::new_v4() },
            serde_json::json!({"type": "analysis_result"}),
        ),
    ];
...
async fn demo_policy_engine() {
    ...
    let test_cases = vec![
        ...
        (
            "Trusted agent reading knowledge",
            Uuid::new_v4(),
            "/knowledge/facts",
            "read",
            EvaluationContext {
        ...
        (
            "Unknown agent accessing sensitive",
            Uuid::new_v4(),
            "/data/sensitive",
            "read",
            EvaluationContext {
    ...
    for (description, subject, resource, action, context) in test_cases {
        ...
        println!(
            "    {} {} - {:?}",
            effect_symbol, description, result.decision
        );
    }
...
async fn demo_event_sourcing() {
    ...
    let events = [
        (
            "AgentCreated",
            serde_json::json!({ "name": "DataAnalyzer", "version": "1.0.0" }),
        ),
        (
            "TaskCompleted",
            serde_json::json!({ "task_id": "analyze-q1", "duration_ms": 1500 }),
        ),
        (
            "KnowledgeAdded",
            serde_json::json!({ "node_id": "insight-1", "type": "pattern" }),
        ),
        (
            "TaskCompleted",
            serde_json::json!({ "task_id": "extract-data", "duration_ms": 800 }),
        ),
        (
            "KnowledgeAdded",
            serde_json::json!({ "node_id": "insight-2", "type": "anomaly" }),
        ),
        (
            "TaskCompleted",
            serde_json::json!({ "task_id": "generate-report", "duration_ms": 2000 }),
        ),
    ];
    ...
    // Subscribe to events
    let sub_id = store.subscribe(vec!["TaskCompleted".into()], "update_dashboard");
    println!(
        "\n  Subscribed to TaskCompleted: {}",
        &sub_id.to_string()[..8]
    );
    ...
    let task_count = events
        ...
        .count();
    let knowledge_count = events
        .iter()
        .filter(|e| e.event_type == "KnowledgeAdded")
        .count();
...
async fn demo_federation() {
    ...
    let remote_registries = vec![
        (
            ...
            "wss://research.edu/registry",
        ),
        (
            "enterprise-net",
            "Enterprise Agent Network",
            "wss://enterprise.corp/registry",
        ),
    ];
    ...
    let results = federation
        .find_agent(AgentCapability::ContentExtraction)
        .await;
