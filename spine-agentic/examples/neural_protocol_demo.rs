...
use spine_agentic::{NeuralProtocol, ProtocolBenchmark, ProtocolDomain};
...
async fn demo_neural_transmission() {
    ...
    let result = protocol
        .transmit(data, ProtocolDomain::RealTime)
        .await
        .unwrap();
...
async fn demo_adaptive_protocols() {
    ...
    let bulk_res = protocol
        .transmit(&large_data, ProtocolDomain::BulkData)
        .await
        .unwrap();
    println!("    [BulkData] 1MB Payload:");
    println!(
        "      • Compression Ratio: {:.2}x",
        bulk_res.compression_ratio
    );
    println!("      • Throughput: {:.2} Mbps", bulk_res.throughput_mbps);

    // 2. Real-Time (Low latency, low compression)
    let rt_res = protocol
        .transmit(&small_data, ProtocolDomain::RealTime)
        .await
        .unwrap();
    println!("\n    [RealTime] 1KB Payload:");
    println!(
        "      • Compression Ratio: {:.2}x",
        rt_res.compression_ratio
    );
    println!("      • Duration: {:?}", rt_res.duration);

    // 3. IoT (Balanced)
    let iot_res = protocol
        .transmit(&small_data, ProtocolDomain::IoT)
        .await
        .unwrap();
    println!("\n    [IoT] 1KB Payload:");
    println!(
        "      • Compression Ratio: {:.2}x",
        iot_res.compression_ratio
    );
...
async fn demo_protocol_benchmarking() {
    ...
    println!(
        "    • Payload Size: {} MB",
        report.data_size / (1024 * 1024)
    );
    println!("    • Neural Duration: {:?}", report.neural_duration);
    println!("    • TCP/TLS Duration: {:?}", report.tcp_duration);
    println!(
        "    • Neural Throughput: {:.2} Mbps",
        report.neural_throughput
    );
    println!(
        "    • TCP/TLS Throughput: {:.2} Mbps",
