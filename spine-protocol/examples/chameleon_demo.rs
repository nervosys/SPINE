...
fn demo_basic_chameleon() {
    ...
    println!(
        "   • First 5 latent values: {:?}",
        &encoded.components[..5.min(encoded.components.len())]
    );
    ...
    println!(
        "   • Same message, new encoding: {:?}",
        &encoded2.components[..5.min(encoded2.components.len())]
    );
    println!("   • New epoch: {}", encoded2.epoch);

    // Check if encodings differ (they should due to evolution)
    let diff: f32 = encoded
        .components
        .iter()
...
fn demo_miras_chameleon() {
    ...
    for (i, msg) in messages.iter().enumerate() {
        ...
        let indicator = if msg.contains("ANOMALY") {
            "⚠️"
        } else {
            "  "
        };
        println!(
            "   {} [{}/{}] Encoded {} dims, anomaly: {:.4}",
            indicator,
            i + 1,
            messages.len(),
            encoded.components.len(),
            anomaly
        );
    }
...
fn demo_variant_selection() {
    ...
    let variants = [
        ...
        (
            MirasVariant::Yaad,
            "🛡️ YAAD",
            "Outlier-robust gradient clipping",
        ),
        (
            MirasVariant::Moneta { p: 2.0 },
            "⏳ MONETA",
            "Lp-norm stability for long sessions",
        ),
        (
            MirasVariant::Memora,
            "⚖️ MEMORA",
            "Probability-constrained updates",
        ),
    ];
