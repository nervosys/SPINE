...
impl SpineBrowser {
    ...
    fn connect(&mut self) {
        ...
        self.rt.spawn(async move {
            let _ = tx
                .send(BrowserEvent::StatusChanged(format!(
                    "Connecting to {}...",
                    addr
                )))
                .await;
            match AgentClient::connect(&addr.to_string()).await {
                Ok(mut client) => {
                    ...
                    *lock = Some(client);
                    let _ = tx
                        .send(BrowserEvent::StatusChanged("Connected".to_string()))
                        .await;
                }
                Err(e) => {
                    let _ = tx
                        .send(BrowserEvent::Error(format!("Connection failed: {}", e)))
                        .await;
                }
            }
        });
                ...
                Ok(_) => {
                    if human_mode {
                        match agent.get_raw_html().await {
                            Ok(html) => {
                                match HumanTranspiler::transpile(&html, "", "") {
                                    Ok(bin) => {
                                        let _ = tx
                                            .send(BrowserEvent::HlsUpdated(format!(
                                                "// Transpiled HLS for {}\n// Instructions: {}\n",
                                                url,
                                            ...
                                            .await;
                                        let _ = tx
                                            .send(BrowserEvent::ContentUpdated(
                                                "Transpilation successful. Executing binary..."
                                                    .to_string(),
                                            ))
                                            .await;
                                        ...
                                        if let Ok(res) = agent.execute_binary(bin).await {
                                            let _ = tx
                                                .send(BrowserEvent::ContentUpdated(format!(
                                                    "Execution Result: {}",
                                                    res
                                                )))
                                                .await;
                                            ...
                                            if let Some(elements_val) = res.get("elements") {
                                                if let Ok(elements) =
                                                    serde_json::from_value::<Vec<WasmElement>>(
                                                        elements_val.clone(),
                                                    )
                                                {
                                                    let _ = tx
                                                        .send(BrowserEvent::ElementsUpdated(
                                                            elements,
                                                        ))
                                                        .await;
                                                }
                                            ...
                                            if let Some(patches_val) = res.get("patches") {
                                                if let Ok(patches) =
                                                    serde_json::from_value::<Vec<VDomPatch>>(
                                                        patches_val.clone(),
                                                    )
                                                {
                                                    if !patches.is_empty() {
                                                        let _ = tx
                                                            .send(BrowserEvent::PatchesApplied(
                                                                patches,
                                                            ))
                                                            .await;
                                                    }
                                        ...
                                        if let Ok(kb) =
                                            agent.query_knowledge("*", vec![], 100).await
                                        {
                                            let _ =
                                                tx.send(BrowserEvent::KnowledgeUpdated(kb)).await;
                                        }
                                        if let Ok(hist) = agent.get_history().await {
                                            let _ =
                                                tx.send(BrowserEvent::HistoryUpdated(hist)).await;
                                        }
                                        if let Ok(caps) = agent.get_capabilities().await {
                                            let _ = tx
                                                .send(BrowserEvent::CapabilitiesUpdated(caps))
                                                .await;
                                        }
                                        ...
                                        if let Ok(ur) = agent.get_ur().await {
                                            ...
                                            let _ = tx
                                                .send(BrowserEvent::ReasoningUpdated(actions))
                                                .await;
                                        }
                                    }
                                    Err(e) => {
                                        let _ = tx
                                            .send(BrowserEvent::Error(format!(
                        ...
                        match agent.get_ur().await {
                            ...
                            Err(e) => {
                                let _ = tx
                                    .send(BrowserEvent::Error(format!("Failed to get UR: {}", e)))
                                    .await;
                            }
                        }
                    }
                    let _ = tx
                        .send(BrowserEvent::StatusChanged("Ready".to_string()))
                        .await;
                }
                Err(e) => {
                    let _ = tx
                        .send(BrowserEvent::Error(format!("Navigation failed: {}", e)))
                        .await;
                }
            }
        });
            ...
            match agent.search(&query).await {
                Ok(results) => {
                    let _ = tx
                        ...
                        .await;
                    let _ = tx
                        .send(BrowserEvent::StatusChanged("Search complete".to_string()))
                        .await;
                ...
                Err(e) => {
                    let _ = tx
                        .send(BrowserEvent::Error(format!("Search failed: {}", e)))
                        .await;
                }
            }
        });
    ...
    fn swarm_search(&mut self) {
        ...
        self.rt.spawn(async move {
            ...
            match agent.swarm_search(&query, 2).await {
                Ok(_) => {
                    let _ = tx
                        .send(BrowserEvent::StatusChanged(
                            "Swarm search initiated across cluster".to_string(),
                        ))
                        .await;
                }
                Err(e) => {
                    let _ = tx
                        .send(BrowserEvent::Error(format!("Swarm search failed: {}", e)))
                        .await;
                }
            }
        });
    ...
    fn delegate_task(&mut self) {
        ...
        self.rt.spawn(async move {
            ...
            match agent.delegate_task(&task, target_id).await {
                ...
                Err(e) => {
                    let _ = tx
                        .send(BrowserEvent::Error(format!("Delegation failed: {}", e)))
                        .await;
                }
            }
        });
        ...
        self.rt.spawn(async move {
            ...
            match agent.propose_knowledge(&key, value, tags).await {
                Ok(_) => {
                    let _ = tx
                        .send(BrowserEvent::StatusChanged(
                            "Knowledge proposed to cluster".to_string(),
                        ))
                        .await;
                }
                Err(e) => {
                    let _ = tx
                        .send(BrowserEvent::Error(format!("Proposal failed: {}", e)))
                        .await;
                }
            }
        });
    ...
    fn create_swarm_plan(&mut self) {
        ...
        self.rt.spawn(async move {
            ...
            match agent.create_swarm_plan(&goal).await {
                ...
                Err(e) => {
                    let _ = tx
                        .send(BrowserEvent::Error(format!("Planning failed: {}", e)))
                        .await;
                }
            }
        });
        ...
        self.rt.spawn(async move {
            ...
            if let Ok(state) = agent.get_agentic_state().await {
                if let Some(variant) = state["miras_variant"].as_str() {
                    ...
                    let _ = tx
                        .send(BrowserEvent::MemoryStateUpdated(
                            variant.to_string(),
                            surprise,
                        ))
                        .await;
                ...
                if let Some(acts_val) = state["speech_acts"].as_array() {
                    if let Ok(acts) = serde_json::from_value::<Vec<spine_agentic::Performative>>(
                        serde_json::Value::Array(acts_val.clone()),
                    ) {
    ...
    fn send_speech_act(&mut self) {
        ...
        self.rt.spawn(async move {
            let target_id = match uuid::Uuid::parse_str(&target_id_str) {
                ...
                Err(_) => {
                    let _ = tx
                        .send(BrowserEvent::Error("Invalid Target Agent ID".to_string()))
                        .await;
                    return;
                }
            };
            ...
            if let Err(e) = agent
                .send_speech_act(target_id, &performative, &content)
                .await
            {
                let _ = tx
                    .send(BrowserEvent::Error(format!(
                        "Failed to send speech act: {}",
                        e
                    )))
                    .await;
            } else {
                let _ = tx
                    .send(BrowserEvent::StatusChanged("Speech act sent".to_string()))
                    .await;
            }
        });
    ...
    fn transfer(&mut self) {
        ...
        self.rt.spawn(async move {
            ...
            match agent.transfer_session(target_node).await {
                ...
                Err(e) => {
                    let _ = tx
                        .send(BrowserEvent::Error(format!("Transfer failed: {}", e)))
                        .await;
                }
            }
        });
        ...
        for elem in self.elements.iter().filter(|e| e.parent_id == parent_id) {
            match elem.tag.as_str() {
                    ...
                    if ui.button(text).clicked() {
                        ...
                        self.rt.spawn(async move {
                            ...
                            let _ = tx
                                ...
                                .await;
                            match agent
                                .handle_event(element_id, "click", serde_json::Value::Null)
                                .await
                            {
                                Ok(patches) => {
                                ...
                                Err(e) => {
                                    let _ = tx
                                        .send(BrowserEvent::Error(format!("Event failed: {}", e)))
                                        .await;
                                }
                            }
                        });
                ...
                "Image" => {
                    ui.label(format!(
                        "[Image: {}]",
                        elem.attributes.get("src").cloned().unwrap_or_default()
                    ));
                }
                "Input" => {
...
impl eframe::App for SpineBrowser {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
            ...
            ui.horizontal(|ui| {
                ...
                if ui.button("Ping").clicked() {
                    ...
                    self.rt.spawn(async move {
                        ...
                        match agent.ping().await {
                            ...
                            Err(e) => {
                                let _ = tx
                                    .send(BrowserEvent::Error(format!("Ping failed: {}", e)))
                                    .await;
                            }
                        }
                    });
                ...
                if ui.button("Latent").clicked() {
                    ...
                    self.rt.spawn(async move {
                        ...
                        let _ = tx
                            .send(BrowserEvent::StatusChanged(
                                "Fetching latent representation...".to_string(),
                            ))
                            .await;
                        match agent.get_latent_ur(128).await {
                            Ok(latent) => {
                                ...
                                let _ = tx
                                    .send(BrowserEvent::StatusChanged(
                                        "Latent vector received".to_string(),
                                    ))
                                    .await;
                            }
                            Err(e) => {
                    ...
                    self.rt.spawn(async move {
                        ...
                        match agent.morph().await {
                            Ok(_) => {
                                let _ = tx
                                    .send(BrowserEvent::StatusChanged(
                                        "Protocol Morphed".to_string(),
                                    ))
                                    .await;
                            }
                            Err(e) => {
                                let _ = tx
                                    .send(BrowserEvent::Error(format!("Morph failed: {}", e)))
                                    .await;
                            }
                        }
                    });
                ...
                if ui.button("Decoy").clicked() {
                    ...
                    self.rt.spawn(async move {
                        ...
                        let _ = tx
                            .send(BrowserEvent::StatusChanged(
                                "Injecting decoy traffic...".to_string(),
                            ))
                            .await;
                        match agent.handler.send_decoy().await {
                            Ok(_) => {
                                let _ = tx
                                    .send(BrowserEvent::StatusChanged("Decoy Injected".to_string()))
                                    .await;
                            }
                            Err(e) => {
                                let _ = tx
                                    .send(BrowserEvent::Error(format!("Decoy failed: {}", e)))
                                    .await;
                            }
                        }
                    });
        ...
        if self.show_hls {
            egui::SidePanel::right("hls_panel")
                .resizable(true)
                .show(ctx, |ui| {
                    ui.heading("HLS Script");
                    egui::ScrollArea::vertical().show(ui, |ui| {
                        ui.add(
                            egui::TextEdit::multiline(&mut self.hls_input)
                    ...
                    if ui.button("Execute HLS").clicked() {
                        ...
                        self.rt.spawn(async move {
                            ...
                            let _ = tx
                                .send(BrowserEvent::StatusChanged(
                                    "Compiling and executing HLS...".to_string(),
                                ))
                                .await;
                            match agent.execute_hls(&script).await {
                                Ok(result) => {
                                    let _ = tx
                                        .send(BrowserEvent::ElementsUpdated(result.elements))
                                        .await;
                                    let _ = tx
                                        .send(BrowserEvent::StatusChanged(
                                            "HLS executed successfully".to_string(),
                                        ))
                                        .await;
                                    ...
                                    for action in result.actions {
                                        match action {
                                            ...
                                            spine_wasm::WasmAction::StoreKnowledge {
                                                key, ..
                                            } => {
                                                let _ = tx
                                                    .send(BrowserEvent::StatusChanged(format!(
                                                        "HLS Storing knowledge: {}...",
                                                        key
                                                    )))
                                                    .await;
                                            }
                    ...
                    egui::ScrollArea::vertical().show(ui, |ui| {
                        ui.add(
                            egui::TextEdit::multiline(&mut self.hls_preview)
                                .font(egui::TextStyle::Monospace)
                                .code_editor(),
                        );
                    });
        ...
        if self.show_latent {
            egui::SidePanel::left("latent_panel")
                .resizable(true)
                .show(ctx, |ui| {
                    ...
                    egui::ScrollArea::vertical().show(ui, |ui| {
                        ui.horizontal_wrapped(|ui| {
                            for &val in &self.latent_vector {
                                ...
                                let (rect, _) = ui
                                    .allocate_at_least(egui::vec2(8.0, 8.0), egui::Sense::hover());
        ...
        if self.show_knowledge {
            egui::SidePanel::left("knowledge_panel")
                .resizable(true)
                .show(ctx, |ui| {
                                    ...
                                    if ui.button("🗑").clicked() {
                                        ...
                                        self.rt.spawn(async move {
                                            ...
                                            if agent.delete_knowledge(&key).await.is_ok() {
                                                ...
                                                {
                                                    let _ = tx
                                                        .send(BrowserEvent::KnowledgeUpdated(kb))
                                                        .await;
                                                }
                                            }
                                        });
                ...
                .show(ctx, |ui| {
                    ...
                    egui::ScrollArea::vertical().show(ui, |ui| {
                        for cmd in &self.session_history {
                            ui.group(|ui| {
                                match cmd {
                                    spine_protocol::BrowserCommand::Navigate { url } => {
                                        ui.label(format!("Navigate: {}", url))
                                    }
                                    spine_protocol::BrowserCommand::GetUR => ui.label("GetUR"),
                                    spine_protocol::BrowserCommand::ExecuteBinary { .. } => {
                                        ui.label("Execute Binary")
                                    }
                                    spine_protocol::BrowserCommand::StoreKnowledge {
                                        key,
                                        value,
                                        ..
                                    } => ui.label(format!("Store: {} = {}", key, value)),
                                    spine_protocol::BrowserCommand::QueryKnowledge {
                                        query,
                                        ..
                                    } => ui.label(format!("Query: {}", query)),
                                    spine_protocol::BrowserCommand::GetSessionHistory => {
                                        ui.label("Get History")
                                    }
        ...
        if self.show_swarm {
            egui::SidePanel::right("swarm_panel")
                .resizable(true)
                .show(ctx, |ui| {
                    ...
                    ui.group(|ui| {
                        ...
                        ui.add(
                            egui::TextEdit::multiline(&mut self.task_description).desired_rows(3),
                        );
        ...
        if self.show_consensus {
            egui::SidePanel::right("consensus_panel")
                .resizable(true)
                .show(ctx, |ui| {
        ...
        if self.show_planning {
            egui::SidePanel::right("planning_panel")
                .resizable(true)
                .show(ctx, |ui| {
                                ...
                                for task in &plan.tasks {
                                    ui.horizontal(|ui| {
                                        let status_color = match task.status {
                                            spine_protocol::TaskStatus::Pending => {
                                                egui::Color32::GRAY
                                            }
                                            spine_protocol::TaskStatus::InProgress => {
                                                egui::Color32::YELLOW
                                            }
                                            spine_protocol::TaskStatus::Completed => {
                                                egui::Color32::GREEN
                                            }
                                            spine_protocol::TaskStatus::Failed => {
                                                egui::Color32::RED
                                            }
                                        };
        ...
        if self.show_neural {
            egui::SidePanel::right("neural_panel")
                .resizable(true)
                .show(ctx, |ui| {
                    ...
                    ui.group(|ui| {
                        ...
                        ui.horizontal(|ui| {
                            ...
                            ui.label(
                                egui::RichText::new(&self.miras_variant)
                        ...
                        ui.horizontal(|ui| {
                            ...
                            ui.add(
                                egui::ProgressBar::new(self.surprise_level)
                                    .text(format!("{:.2}", self.surprise_level)),
                            );
                        });
                    });
                        ...
                        egui::ComboBox::from_label("Modality")
                            ...
                            .show_ui(ui, |ui| {
                                ui.selectable_value(
                                    ...
                                    spine_agentic::ProtocolDomain::BulkData,
                                    "BulkData",
                                );
                                ui.selectable_value(
                                    &mut self.selected_domain,
                                    spine_agentic::ProtocolDomain::Text,
                                    "Text",
                                );
                                ui.selectable_value(
                                    &mut self.selected_domain,
                                    spine_agentic::ProtocolDomain::Image,
                                    "Image",
                                );
                                ui.selectable_value(
                                    &mut self.selected_domain,
                                    spine_agentic::ProtocolDomain::Audio,
                                    "Audio",
                                );
                                ui.selectable_value(
                                    &mut self.selected_domain,
                                    spine_agentic::ProtocolDomain::Video,
                                    "Video",
                                );
                                ui.selectable_value(
                                    &mut self.selected_domain,
                                    spine_agentic::ProtocolDomain::StructuredData,
                                    "StructuredData",
                                );
                                ui.selectable_value(
                                    &mut self.selected_domain,
                                    spine_agentic::ProtocolDomain::Code,
                                    "Code",
                                );
                                ui.selectable_value(
                                    &mut self.selected_domain,
                                    spine_agentic::ProtocolDomain::NeuralWeights,
                                    "NeuralWeights",
                                );
                            });
        ...
        if self.show_communication {
            egui::SidePanel::left("comms_panel")
                .resizable(true)
                .show(ctx, |ui| {
                        ...
                        if self.speech_acts.is_empty() {
                            ...
                            for act in &self.speech_acts {
                                ui.group(|ui| {
                                    ui.horizontal(|ui| {
                                        ui.label(
                                            egui::RichText::new(format!("{:?}", act.speech_act))
                                                .color(egui::Color32::GOLD),
                                        );
                                        ui.label(format!(
                                            "from: {}",
                                            act.sender.to_string().get(0..8).unwrap_or("...")
                                        ));
                                    });
